...
non-sealed class YellowSign extends Faction {
    ...
    static int getCombatUndead(Core core) {
        ArrayList<Location> Zombies = core.factionBase.getFactionFromEnum(FactionType.YellowSign)
                .getEntitySetByName("Undead").positions;
        int combat = 0;
        for (int i = 0; i < Zombies.size(); i++) {
            if (Zombies.get(i) == core.var.battleLocation)
                combat++;
        }
        return combat - 1;
    }
    ...
    static int getCombatByakhee(Core core) {
        ArrayList<Location> Zombies = core.factionBase.getFactionFromEnum(FactionType.YellowSign)
                .getEntitySetByName("Byakhee").positions;
        int combat = 0;
        for (int i = 0; i < Zombies.size(); i++) {
            if (Zombies.get(i) == core.var.battleLocation)
                combat++;
